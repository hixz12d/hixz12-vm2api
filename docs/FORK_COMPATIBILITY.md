...
## 上游主分支同步（2d7c16f / v1.3.39）

本次从 `e6a8137` 同步至上游 `2d7c16f`，包含 v1.3.31–v1.3.39：会话与 native slot 粘性修复、额度窗口到期探测、Fable 模型能力筛选、429/529 硬冷却与空响应重试、经中继的官方 Claude Code 识别、2.1.280 cch/beta 适配、播种环境策略，以及面板批量重装和同时拉取 kernel/cli-node。仓内预编译 kernel 与 cli-node 均随上游更新。

接受上游缓存修复：支持 `role=system` 的模型保持末尾提醒在 messages 中的原位置，包含经中继后被识别为第三方的请求；Haiku 仍搬移提醒。替换 fork 旧的“末尾 system 总是移入 system[]”行为，避免每轮改变系统提示词前缀。历史内容与上下文计数保持稳定，Node 的历史 user 缓存边界继续按轮次前移；CLI TTL 仍统一 5m，自动断点开关、断点数上限和实际 5m 计费保留。未进行真实 1h 工具循环验证，因此不解除这一限制。

调度采用上游硬冷却、额度错误还原和 Fable 成功反馈，同时保留 fork 的 `SessionQueue` 取消语义、标题子会话、工具输出蒸馏排除、真实 SSE 终止事件校验、watchdog 与槽位配置属主保护。特征采集继续使用 fork 已验证的 Docker guest 采集器与结构校验；上游新增的 shell 读取器及其测试保留。槽位镜像和部署策略不变。

本次仅同步源码 fork，未部署生产服务。仓内新版二进制需要后续部署并同步槽位后才会实际生效；源码测试不能替代真实推理、缓存命中和在线槽位更新验证。
